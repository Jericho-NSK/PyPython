...
class Final(Toplevel):
    ...
    def create_menu(self, window, game, win):
        ...
        Button(master=self, text='New game', font=self._font_buttons, bd=6, fg='green',
               command=lambda: Menubar.new_game(window, game, final=self)).grid(row=5, column=1, ipadx=40, ipady=5)
        Button(master=self, text='Exit', font=self._font_buttons, bd=6, fg='red',
               command=self.quit).grid(row=5, column=3, ipadx=60, padx=30, ipady=5)
