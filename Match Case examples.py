# def calculate(a: int | float, b: int | float, operator: str) -> str | int | float:  # pattern matching - сопоставление шаблонов
...
#             _ = a + b  # нет return, поэтому переход в основной return. остальные шаблоны не проверяются если этот возвращает True
#         case '-':
#             _ = a - b
#         case _: # wildcard, выполняется если не совпали шаблоны выше
#             return 'Unknown operation'
#     return _  # выполняется если нет return в case, который вернул True. например если вместо return вызывается print, прописан pass.
...
#             print(f'Shoot by coordinates {coordinates[-1]}')
...
#     run_action('shoot 100 200 3 444'.split())
...
#             # расстояние сохраняется в value, а направление проверяется и сохраняется в action. при это новый экземпляр НЕ создается.
...
#             print(f'Moving {action} on {value} px')
#         case UserInput('left' | 'right' as action, value):  # если прописан метод __match_args__, то при проверке в case атрибуты будут проверяться
#             # именно в таком порядке. указывать их названия action и value уже не нужно
#             print(f'Moving {action} on {value} px')
...
#             return 'Error'
#     return f'{name} {surname}'  # выполняется если нет return в case, который вернул True. например если вместо return вызывается print или pass.
#             # сюда попадают переменные из УСПЕШНОГО case, условия в котором вернули True, тогда сохранились переменные из этого case
#
#
